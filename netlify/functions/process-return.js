...
exports.handler = async (event) => {
  ...
  try {
    const { booking_id, return_token, photo_url } = JSON.parse(event.body);

    // Buchung laden und Token prüfen
    ...
    if (error || !booking) return { statusCode: 404, headers, body: JSON.stringify({ error: 'Buchung nicht gefunden' }) };
    if (booking.status === 'returned') return { statusCode: 400, headers, body: JSON.stringify({ error: 'Buchung bereits abgeschlossen' }) };
    ...
    // Verspätungsaufpreis automatisch abbuchen
    if (lateFeeAmount > 0 && booking.stripe_payment_method_id && booking.stripe_customer_id) {
      try {
        const latePi = await stripe.paymentIntents.create({
          amount: Math.round(lateFeeAmount * 100),
          currency: 'eur',
        ...
        // Buchung trotzdem abschließen – Admin muss manuell nachverfolgen
      }
    }
    ...
    // Anhänger wieder verfügbar schalten
